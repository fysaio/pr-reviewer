...
async def github_webhook(request: Request):
    ...
    if event == "pull_request":
        ...
        if action in ("opened", "synchronize"):
            ...
            # Index the repo on first PR open, skip on synchronize
            if action == "opened":
                print(f"[Indexer] Indexing repo {repo}...")
                index_repo(repo, token)

            # Search for relevant context based on the diff
            ...
            print(f"Fetched diff. Running Gemini review...")
            findings = review_pr(diff, pr_desc, context_files)
